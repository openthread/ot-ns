...
Change the OpenThread (OT) executable, or shell script, for a particular node type as provided in the first argument (ftd, mtd, br, rcp or host). The path-or-filename is provided in the second argument and will replace the current default executable for that node type. If only the first argument is given, the current executable for this node type will be listed and no change is made. If only a filename is given, without full path, the executable will be located using the search paths listed under `Executables search path`.
